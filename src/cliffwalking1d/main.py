...
sys.path.append("../")
from cliffwalking1d import CliffWalking1D
...
RUN_ID = ("cliff-GAMMA(%.2f)-ALPHA(%f)-MEMSIZE(%d)-EPISODES(%d)-BATCHSIZE(%d)-REPLACE_AFTER(%d)-PER(%r)" % (GAMMA, ALPHA, MAX_MEMORY_SIZE, NUM_EPISODES, BATCH_SIZE, REPLACE_AFTER, USE_PER))
...
agent = Agent(Net=Net, input_size=1, output_size=env.action_space.n, gamma=GAMMA, alpha=ALPHA, max_memory_size=MAX_MEMORY_SIZE, num_episodes=NUM_EPISODES, replace_after=REPLACE_AFTER, use_per=USE_PER, per_a=1)
# Filling memory
...
for i_episode in range(NUM_EPISODES):
    ...
    if (i_episode%n_episode == 0 and i_episode != 0):
        print("Using PER: ", USE_PER, "Learning at episode", str(i_episode)+"/"+str(NUM_EPISODES), "Average reward last 50 episodes: ", sum(rewards[-n_episode:])/n_episode, "Buffer size: ", agent.PER.tree.entries if USE_PER else len(agent.buffer.buffer), "Average time used learning:", sum(time_learned[-n_episode:])/n_episode)
...
if SAVE_MODEL: torch.save(agent.Q_loc.state_dict(), "trainedmodels/trainedmodel-%s.pt" % (RUN_ID))
